...
app.whenReady().then(async () => {
  ...
  assert.match(await evaluate(`document.querySelector('#memory option:checked').textContent`), /Auto · 4 Go/)
  assert.equal(await evaluate(`document.querySelector('.launch-context')`), null)
  rejectMemory = true
  await evaluate(`{ const select=document.querySelector('#memory'); select.value='6144'; select.dispatchEvent(new Event('change',{bubbles:true})) }`)
  await delay(60)
  assert.equal(await evaluate(`document.querySelector('#memory').value`), 'auto')
  ...
  assert.ok(snapshot.settings.top >= 0 && snapshot.settings.bottom <= snapshot.height)
  await evaluate(`document.querySelector('dialog').close()`)
  await capture('14-discord-pending', { access: 'pending', busy: true, cancellable: true, operation: 'discord', message: 'Termine la connexion dans ton navigateur.' })
  await evaluate(`document.querySelector('.gate-actions button').click()`)
  await delay(60)
  assert.equal(actions.at(-1).name, 'openDiscordLogin')
  await capture('15-discord-denied', { access: 'denied', message: 'Ce compte n’est pas autorisé à accéder au launcher.' })
  await evaluate(`document.querySelector('.gate-copy .primary').click()`)
  await delay(60)
  assert.equal(actions.at(-1).name, 'discordLogin')
  await capture('16-access-outage', { access: 'unavailable', messageType: 'error', message: 'Le service de connexion est indisponible.' })
  assert.equal(await evaluate(`document.querySelectorAll('.gate-copy button').length`), 1)
  await evaluate(`document.querySelector('.gate-copy .primary').click()`)
  await delay(60)
  assert.equal(actions.at(-1).name, 'refresh')
  await capture('17-crash', { ...ready, messageType: 'error', message: 'Minecraft s’est fermé avec une erreur. Les journaux sont dans le dossier du jeu.' })
  await evaluate(`document.querySelector('.error-panel button').click()`)
  await delay(60)
  assert.equal(actions.at(-1).name, 'openLogs')
  await capture('18-notes', ready)
  await evaluate(`document.querySelector('.release-notes summary').click()`)
  await evaluate(`document.querySelector('.notes-body button').click()`)
  await delay(60)
  assert.equal(actions.at(-1).name, 'openReleaseNotes')
  await capture('18-notes-expanded')
  await evaluate(`document.querySelector('.release-notes summary').click(); document.querySelector('.icon-button').click()`)
  await evaluate(`document.querySelectorAll('.maintenance-actions button')[1].click()`)
  await delay(60)
  assert.equal(actions.at(-1).name, 'openLogs')
  await evaluate(`document.querySelectorAll('.maintenance-actions button')[2].click()`)
  await delay(60)
  assert.equal(actions.at(-1).name, 'install')
  assert.equal(await evaluate(`document.querySelector('dialog').open`), false)
